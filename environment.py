...
class TheMindEnv:
    ...
    def send_message(self, player_id: int, message: str, act: str = "wait"):
        """Añade un mensaje al canal de comunicación junto con la acción tomada."""
        if message and message.strip():
            self.state.messages.append({
                "player": player_id,
                "text":   message.strip()[:200],
                "act":    act if act in ("play", "wait", "star") else "wait",
            })
        elif act in ("play", "star"):
            # Acción sin mensaje verbal: registrar para el contexto
            self.state.messages.append({
                "player": player_id,
                "text":   "",
                "act":    act,
